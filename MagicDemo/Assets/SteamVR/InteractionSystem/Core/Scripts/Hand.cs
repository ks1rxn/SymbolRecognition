...
namespace Valve.VR.InteractionSystem {
	...
	public class Hand : MonoBehaviour {
		...
		public const AttachmentFlags defaultAttachmentFlags = AttachmentFlags.ParentToHand | AttachmentFlags.DetachOthers | AttachmentFlags.DetachFromOtherHand | AttachmentFlags.SnapOnAttach;
			...
			set {
				if (_hoveringInteractable != value) {
					if (_hoveringInteractable != null) {
						...
						if (_hoveringInteractable != null) {
							BroadcastMessage("OnParentHandHoverEnd", _hoveringInteractable, SendMessageOptions.DontRequireReceiver); // let objects attached to the hand know that a hover has ended
						}
					}
					...
					if (_hoveringInteractable != null) {
						...
						if (_hoveringInteractable != null) {
							BroadcastMessage("OnParentHandHoverBegin", _hoveringInteractable, SendMessageOptions.DontRequireReceiver); // let objects attached to the hand know that a hover has begun
						}
					}
				}
			}
		...
		private void UpdateHovering() {
			...
			Physics.OverlapBoxNonAlloc(hoverSphereTransform.position - new Vector3(0, flScaledSphereRadius * boxMult - flScaledSphereRadius, 0), new Vector3(flScaledSphereRadius, flScaledSphereRadius * boxMult * 2.0f, flScaledSphereRadius), overlappingColliders, Quaternion.identity, hoverLayerMask.value);
...
//				}
		...
		private void UpdateDebugText() {
			if (showDebugText) {
				...
				debugText.text = string.Format("Hovering: {0}\n" + "Hover Lock: {1}\n" + "Attached: {2}\n" + "Total Attached: {3}\n" + "Type: {4}\n", hoveringInteractable ? hoveringInteractable.gameObject.name : "null", hoverLocked, currentAttachedObject ? currentAttachedObject.name : "null", attachedObjects.Count, GuessCurrentHandType());
			} else {
				if (debugText != null) {
		...
		private void InitController(int index) {
			if (controller == null) {
				...
				BroadcastMessage("OnHandInitialized", index, SendMessageOptions.DontRequireReceiver); // let child objects know we've initialized
			}
		}
		...
		public override void OnInspectorGUI() {
			...
			if (hand.otherHand) {
				...
				if (hand.startingHandType == Hand.HandType.Any && hand.otherHand.startingHandType != Hand.HandType.Any) {
					EditorGUILayout.HelpBox("This is an any-handed Hand but otherHand is not an any-handed Hand.", MessageType.Warning);
				}
			}
		}
	}
